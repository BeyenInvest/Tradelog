...
import { computeEquityCurve, computeOutcomeCounts } from "@/lib/stats";
...
export function PeriodicReviewForm({ periodType, review, trades, onSubmit, onClose }: PeriodicReviewFormProps) {
  ...
  const takenPreview = tradesInPeriod.filter((t) => t.trade_evaluation !== "Missed trade");
  const missedPreview = tradesInPeriod.filter((t) => t.trade_evaluation === "Missed trade");
          ...
          <div className="grid grid-cols-2 gap-4">
            {periodType === "month" && (
              <div className="flex flex-col gap-1.5">
                ...
                <select className="input" value={periodeNummer} onChange={(e) => setPeriodeNummer(Number(e.target.value))}>
                  {MONTH_NAMES.map((m, i) => (
