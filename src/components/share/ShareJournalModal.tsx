...
import {
  ...
  createShareLink, expiryFromChoice, listShareLinks, revokeShareLink, shareLinkStatus, shareUrl,
...
/**
 * Owner-side management of share-links for the active journal (Fase M, beta-gated
 * by the caller): create a token URL a coach can open without an account, copy it,
 * and revoke it. The token is a capability — the copy in this modal says so
 * explicitly, so nobody mails a link around thinking it's login-protected.
 */
export function ShareJournalModal({
  ...
  useEffect(() => {
    ...
    // `t` deliberately not a dep: a language switch must not refetch the list
    // (and could race an in-flight revoke) — same pattern as AdminUserDetailPage.
  ...
  async function handleCreate() {
    ...
    setError(null);
    try {
      const link = await createShareLink(userId, methodologyId, expiryFromChoice(expiry));
  ...
  async function copyLink(link: ShareLink) {
    try {
      await navigator.clipboard.writeText(shareUrl(window.location.origin, link.token));
      setCopiedId(link.id);
      if (copyResetTimer.current) clearTimeout(copyResetTimer.current);
  ...
  return (
    <>
      <Modal labelledBy="share-journal-title" maxWidthClass="max-w-lg" scroll onClose={onClose}>
        {(requestClose) => (
          <>
            <div className="flex items-center justify-between mb-1">
              <h2 id="share-journal-title" className="font-display text-xl italic text-ink">
                {t("share.modalTitle")}
              </h2>
            ...
            <p className="font-body text-sm text-muted mb-5">{t("share.modalSubtitle")}</p>
