...
/**
 * One share-link (Fase M / 0040): a capability token that shows this user's
 * journal read-only to anyone holding the URL — no account needed. Owner-managed
 * via RLS; the anonymous viewer only ever reads through the get_shared_journal RPC.
 */
export interface ShareLink {
  id: string;
  user_id: string;
  /** Which journal the link exposes. null = the legacy/unscoped journal (trades.methodology_id is null). */
  methodology_id: string | null;
  /** Only 'journal' exists today; review-sharing is a later slice (see 0040). */
  scope: "journal";
