...
**2026-09-18 middag (3) — sluitdatum + W-D-D-snapshotbug na owner-test (branch `tv-ext-close-and-snapshots`):** twee owner-vondsten. (1) **Gesloten trade zonder sluitdatum:** een Win/Loss/BE-log uit het paneel liet `datum_sluiting` leeg — de post-hoc-mode kende het veld al maar niemand vulde het. Nu stuurt het paneel `closeTimeUtcSec` (de laatste bar van de chart = chart-"nu", replay-bewust — zelfde regel als closeFlow) mee en leidt `buildTradePayload` er de sluitdatum uit af in de profiel-tijdzone; een expliciete mode-datum wint, een close-vóór-open (verkeerde lezing) degradeert naar leeg i.p.v. fout. (2) **W-D-D-snapshotbug:** `runSnapshotCycle` vergeleek elk slot met de resolutie van cycle-**start**; stond de chart op 4H, dan dacht het 4H-slot "sta ik al op" terwijl het D-slot 'm net op Daily had gezet → tweede Daily-screenshot in het 4H-slot. Fix: een meeschuivende `current`-resolutie. Bijvangst: `serve-ext-harness.mjs` accepteert nu root/poort-argumenten (harnas draaien vanuit een tijdelijke worktree). 655 tests groen (regressietests voor beide bugs); harnas bewijst de request met `closeTimeUtcSec` uit de laatste bar én `legacy.cc = "07"`.

**2026-09-18 middag (2) — CC-fix na owner-test van PR #17 (branch `tv-ext-cc-fix`):** owner zag entry 11:00 → CC "15": de eerste afleiding nam de close van de candle waarin de entry valt, maar de WPM-workflow is "de close bevestigt, dán instappen" — CC hoort dus de **meest recente 4H-close op of vóór de entry** te zijn (11:00 → 11, 14:32 → 11, vóór 03:00 → de 23 van gisteren). `ccFromTime` omgedraaid. Tweede owner-punt: de CC-select is helemaal uit het paneel (clutter) — CC is nu machine-owned: `syncCc` zet 'm rechtstreeks in values (of verwijdert 'm zonder bruikbare tijd → server-default), `legacyFromValues` laat "cc" expliciet toe naast de gerenderde velden, en de `LegacyForm.sync()`-machinerie van de prefill-ronde is weer weg (bestond alleen voor die select). 648 tests groen (semantiek-tests omgeschreven); harnas bewijst: geen CC-veld meer, request draagt `legacy.cc = "07"` bij bartijd 07:00.

**2026-09-18 middag — frictie-ronde na owner-feedback (branch `tv-ext-frictie`):** twee owner-punten ("alles zo clean & minimalistisch mogelijk"). (1) **CC-prefill:** het 4H Candle Close-veld vult zichzelf uit de entry-tijd. Semantiek uit de DB afgeleid (compute_sessie gebruikt `cc` als sluituur in de profiel-tijdzone): CC = de close van de 4H-candle waarin de entry valt; exact op een slot → de candle die dan opent, na 23:00 wikkelt naar 03 (volgende dag). Pure functie `ccFromTime` (fields.ts), bar-tijd → wall-clock via `TargetsInfo.timezone` (nieuw, SW vult 'm uit profiles; fallback Europe/Brussels zoals de sessie-trigger), handmatige tijd-input doet mee. Prefill-hygiëne: alleen een leeg veld of de vorige auto-waarde wordt gezet; een eigen keuze (of wissen) zet de prefill definitief uit (`ccTouched`); DOM-update via nieuwe `LegacyForm.sync()` (geen re-render, geen focusverlies). Bijvangst: tot nu ging bij een onaangeroerd veld stilletjes de quickLog-default "11" mee — de prefill maakt die kolom dus ook nog eens juist. (2) **Doel + resultaat samengevoegd:** de aparte "Modus"-sectie (Live/Backtest-toggle) is weg; onder de journal-keuze staat nu één rij **Running | Win | Loss | BE** (`result`-state vervangt mode+outcome; live-default Running, backtest-project-default bewust leeg, Running blijft ook daar kiesbaar). Win/Loss/BE tonen het resultaat%-veld met **auto-voorstel** `suggestedResultPct` (priceMath): "liep af zoals getekend" — Win = R:R × risk%, Loss = −risk%, BE = 0, lege risk volgt de 1%-default; voorstel-hint verdwijnt zodra de user typt, zelfde touched-hygiëne als CC (review-fix Fable: zonder `resultTouched` vulde een gewist veld zich meteen weer). CSS: opt-in `.by-outcomes.is-quad`, Running in neutraal goud. Copy NL/EN opgeschoond (mode-keys weg, "Running" is een leenwoord zoals Win/Loss/BE). Werkverdeling per model-regel: pure laag + review Fable (098c265), paneel-UI Opus-agent. **648 tests groen**, paneel-bundle 146,1 KB; harnas-run bewijst CC "11" uit bar-tijd 07:00, Win-voorstel 2 → risk 0.5 → 1, wis-vast-blijft, project-default met validatie, submit "Trade gelogd".

**2026-09-18 — snapshot-gebaar-fix + F5a (owner-go "bouw F5", branch `tv-ext-f5`):** (1) **Het activeTab-gebaar per tab is uit de snapshot-flow** — owner-frustratie ("telkens bij een nieuwe sessie"). Runtime-spike in de browser-pane bewees dat `TradingViewApi.takeClientScreenshot()` een Promise<HTMLCanvasElement> van alléén de chart teruggeeft (niet tainted, `toDataURL` werkt). Dat is nu het primaire beeld-pad: page-world-commando `take-screenshot` (tvMain → bridge, eigen 10s-timeout) → PNG-data-URL → SW. Geen permissie-gebaar, geen crop. `captureVisibleTab`+crop blijft als fallback bij TV-drift; alleen dáár bestaat "needs-gesture" nog. `SnapshotDeps` versimpeld naar één `capture()`-dep. (2) **S1-spike = GO:** `activeChart().getSeries().data().bars().last()` levert `[timeSec, o, h, l, c, vol]` (close matcht de live koers exact) en `isInReplay` is leesbaar → `ChartState.lastBar` (per-veld-degradatie zoals altijd). (3) **F5a gebouwd** (Fable): `realizedR`/`resultaatPctFromExit` in priceMath (richting-symmetrische formule, harde weigering bij SL-verkeerde-kant; lege risk_pct volgt de app-brede 1%-default i.p.v. §7.1's strengere eis — R ≡ resultaat% voor de vlakke-1%-workflow), `deriveOutcome` hergebruikt uit de import-laag (mét de gedeelde BE-epsilon 0.005 — §7.1 zei "exact 0", de gedeelde helper wint), `listOpenTrades`/`updateTrade` in ExtensionDb (expliciete user_id-filter), `closeFlow.ts` (listOpenTradesForSymbol, closeTradeFromChart met missed-trade-weigering + close-before-open-guard + replay-veilige sluitdatum uit bar-tijd, previewClose voor de UI), `updateLoggedTradeByRef` (gedeelde payloadbouw met logTradeFromChart), 3 nieuwe SW-berichten. esbuild-panelbuild kent nu de `@`-alias (nodig voor gedeelde modules; build:ext(:dev) in sync gehouden). 613 tests groen. (4) **F5b af** (Opus-agent, zelfde dag): sluit-sectie vlak onder Chart, formulier met exit-prefill uit de laatste candle + live preview via previewClose (outcome-badge afgeleid, nooit kiesbaar), handmatig-%-pad met datumguard, evaluatie zonder Missed, MAE/MFE alleen bij track_exit, "Nog aanpassen" via update-trade mét screenshot-merge (bewerking veegt de vier kolommen niet leeg), copy NL/EN, harnas-stubs. Review-vondst van de agent aan de wortel gefixt (Fable): `wallClockInTimezone` naar eigen mini-module `src/lib/wallClock.ts` — closeFlow trok via tradePayload heel zod het content-script in (paneel-bundle 277 KB → 141,6 KB). Eindstand: **639 tests groen**, harnas-run bewijst prefill → 1.18 R/1.18%/Win → correcte close-request met bar-tijd. Branch `tv-ext-f5` (1b179f3, gestapeld op tv-ext-legacy-fields).
...
**Status: CODE-COMPLEET op branch `tv-ext-f5` (18-09, 1b179f3)** — S1-let ✅ GO (runtime: `getSeries().data().bars().last()` + `isInReplay`), F5a ✅ (rekenpad, open-trades, close, edit-by-import_ref, unit-getest), F5b ✅ (paneel-UI, Opus; runtime geverifieerd in het dev-harnas). Rest: owner-test in echte Chrome + PR-merge (na #12). Geen migratie nodig (`exit_price` bestaat al sinds 0058). Bouwafwijkingen t.o.v. het ontwerp hieronder: (a) outcome via de gedeelde `deriveOutcome` mét de app-brede BE-epsilon (0.005), niet "exact 0" — één bron van waarheid wint; (b) lege `risk_pct` blokkeert het exit-prijs-pad níet maar volgt de app-brede 1%-default (zelfde conventie als riskPct() in stats/core) — zonder prijzen blijft alleen handmatig resultaat over; (c) MAE/MFE alleen zichtbaar als het journal `track_exit` heeft (zelfde opt-in als de web-form) — dat beantwoordt open beslispunt 2.
