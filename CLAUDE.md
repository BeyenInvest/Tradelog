...
- **Uitzondering (owner-besluit 2026-09-15/16): de TradingView-extensie.** Leidend plan = `docs/plan-tv-extensie-engines.md` (bouwlog in §6). Stand 2026-09-23: **F1–F5 + un-gate volledig gemerged en live op main/prod** — koppel-endpoint (`api/extension-link.ts`), Settings-kaart, MV3-extensie in `extension/` (auth, chart-adapter, log-paneel, snapshots, close-from-chart; `npm run build:ext`, laden = unpacked op `extension/`), security-review gedaan (closed shadow root), open voor álle ingelogde leden (auth + per-user rate-limit blijven). Laatste gemergde ronde = **v0.2.8** (PR #40: unified ScreenshotField, slot-namen 0060, entry-tijd/sluitdatum-fixes). Store: unlisted op de Chrome Web Store, **zip v0.2.8 geüpload, review pending** — leden kunnen installeren zodra die door is (⚠️ Store-console is niet automatiseerbaar, owner-werk). **In aanbouw: per-journal instelbare snapshot-timeframes** (plan-doc §8, branch `tv-ext-snapshot-timeframes`, migratie **0061**, ext v0.2.9) — owner draait 0061 vóór de deploy. Model-per-fase geldt (Fable: auth/adapter/migraties/security, Opus: UI).
...
- Migratienummers: nooit hergebruiken; elke migratie werkt `supabase/schema.sql` mee bij. **0059** (WPM fase-retirement) en **0060** (screenshot-slot-namen) zijn gedraaid op prod. **0061** = per-journal snapshot-timeframes voor de extensie (`methodologies.screenshot_timeframes` + fork-fix, branch `tv-ext-snapshot-timeframes`; owner draait 'm vóór deploy). Eerstvolgend vrij nummer = **0062**.
